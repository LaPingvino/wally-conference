...
func (svc *Service) HandleWebhook(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("Received LiveKit webhook: %s", event.GetEvent())
	...
	default:
		log.Printf("Ignoring unhandled webhook event type: %s", event.GetEvent())
	}
...
func (svc *Service) handleParticipantLeft(ctx context.Context, evt *livekit.WebhookEvent) {
	...
	log.Printf("participant_left: identity=%s", identity)

	session, err := GetSessionByIdentity(svc.DB, identity)
	if err != nil {
		log.Printf("Error looking up session by identity: %v", err)
		return
	}
	if session == nil {
		log.Printf("No guest session found for identity %s (may be a Matrix user)", identity)
		return
	}

	// Clear the call.member state event
	if err := ClearCallMember(ctx, svc.Client, id.RoomID(session.RoomID), session.StateKey); err != nil {
		log.Printf("Failed to clear call.member for session %s: %v", session.ID, err)
	} else {
		log.Printf("Cleared call.member for session %s (room=%s)", session.ID, session.RoomID)
	}

	if err := DeleteSession(svc.DB, session.ID); err != nil {
		log.Printf("Failed to delete session %s: %v", session.ID, err)
	} else {
		log.Printf("Deleted guest session %s", session.ID)
	}
}
...
func (svc *Service) handleRoomFinished(ctx context.Context, evt *livekit.WebhookEvent) {
	...
	log.Printf("room_finished: lk_room=%s", roomName)

	sessions, err := GetSessionsByLKRoom(svc.DB, roomName)
	if err != nil {
		log.Printf("Error looking up sessions for LK room: %v", err)
		return
	}
	if len(sessions) == 0 {
		log.Printf("No guest sessions found for LiveKit room %s", roomName)
		return
	}
	...
	log.Printf("Cleaned up %d guest sessions for LiveKit room %s", len(sessions), roomName)
}
